...
plt.grid()
plt.show()
plt.savefig(f"./results/figures/DQL_{env_config['num_episodes']}eps_{env_config['capacity']}cap_{env_config['forecast']}forecast")

actions_plot = np.array(actions_tot[-1:][:24*4]).flatten()
plt.scatter(range(len(actions_plot)),actions_plot)
plt.title('Actions used in the last 3 episodes')
